        ...
        return {
            upload: async file => {
                ...
                if (
                    ...
                    const prefix = config.prefix && (!config.customDomain || config.customDomain === '-') ? config.prefix.trim() : '';
                    const objectKey = `${prefix}${path}${file.hash}${file.ext}`;
                    const s3url = `https://${config.params.Bucket}.s3.${config.region}.amazonaws.com`;

                    file.url = (config.customDomain && config.customDomain !== '-') ? `${config.customDomain}/${objectKey}` : `${s3url}/${objectKey}`;
