...
const generateImages = async (file, config) => {
  ...
  const { thumbnails, webp, webpConfig, quality } = config;
  ...
  const imagesToCreate = thumbnails.map(async item => {
    ...
    switch (file.ext.toLowerCase()) {
      case '.png':
        images.push(
          await sharp(buffer).resize(options || {})
            .png({ quality: parseInt(quality), ...(outputOptions || {}) }).toBuffer()
            .then(data => ({
              buffer: data,
              mime: file.mime,
              ext: file.ext,
      ...
      case '.webp':
        images.push(
          await sharp(buffer).resize(options || {})
            .webp({ quality: parseInt(quality), ...(outputOptions || {}) }).toBuffer()
            .then(data => ({
              buffer: data,
              mime: file.mime,
              ext: file.ext,
      ...
      case '.jpeg':
        images.push(
          await sharp(buffer).resize(options || {})
            .jpeg({ quality: parseInt(quality), ...(outputOptions || {}) }).toBuffer()
            .then(data => ({
              buffer: data,
              mime: file.mime,
              ext: file.ext,
    ...
    if (webp && file.mime !== 'image/webp') {
      images.push(
        await sharp(buffer).resize(options || {})
          .webp({ quality: parseInt(quality), ...(webpConfig || {}) }).toBuffer()
          .then(data => ({
            buffer: data,
            mime: 'image/webp',
            ext: '.webp',
